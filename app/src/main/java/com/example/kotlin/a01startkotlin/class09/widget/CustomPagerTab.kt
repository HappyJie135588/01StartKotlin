...
class CustomPagerTab @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) : PagerTabStrip(context, attrs) {
    ...
    private var txtSize = 15

    init {
        //初始化时从attrs.xml读取CustomPagerTab的自定义属性
        if (attrs != null) {
            val attrsArray = getContext().obtainStyledAttributes(attrs, R.styleable.CustomPagerTab)
            //从布局文件中获取新属性textColor的数值
            txtColor = attrsArray.getColor(R.styleable.CustomPagerTab_textColor, txtColor)
            //从布局文件中获取新属性textSize的数值
            txtSize = attrsArray.getDimensionPixelSize(R.styleable.CustomPagerTab_textColor, txtSize)
            attrsArray.recycle()
        }
        //应用布局文件的textColor文本颜色
        setTextColor(txtColor)
        //应用布局文件的textSize文本大小
        setTextSize(TypedValue.COMPLEX_UNIT_SP, txtSize.toFloat())
    }
}