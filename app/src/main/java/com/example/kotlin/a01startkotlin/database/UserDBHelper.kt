package com.example.kotlin.a01startkotlin.database
...
class UserDBHelper(var context: Context, private var DB_VERSION: Int = CURRENT_VERSION) : ManagedSQLiteOpenHelper(context, DB_NAME, null, DB_VERSION) {
    companion object {
        ...
        var CURRENT_VERSION = 2//当前的最新版本，如有表结构变更，该版本号要加一
...
//                ",password VARCHAR" +
    ...
    fun insert(info: UserInfo): Long {
        val infoArray = mutableListOf<UserInfo>(info)
        return insert(infoArray)
    }

    private fun insert(infoArray: MutableList<UserInfo>): Long {
        var result: Long = -1
